...
RETURNS TABLE(position bigint, total_users bigint, vendas_count bigint)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  IF current_period_id IS NULL THEN
    RETURN QUERY
    SELECT 1::bigint as position, 1::bigint as total_users, 0::bigint as vendas_count;
    RETURN;
  END IF;
  ...
  IF ranking_type = 'consultor' THEN
    ...
    ranking_calc AS (
      SELECT 
        ...
        COUNT(CASE WHEN ac.sales_count > (SELECT sales_count FROM user_sales) THEN 1 END) + 1 as position
  ...
  ELSIF ranking_type = 'gerente' THEN
    ...
    ranking_calc AS (
      SELECT 
        COUNT(*) as total_users,
        COUNT(CASE WHEN ag.mentions_count > (SELECT mentions_count FROM user_mentions) THEN 1 END) + 1 as position
