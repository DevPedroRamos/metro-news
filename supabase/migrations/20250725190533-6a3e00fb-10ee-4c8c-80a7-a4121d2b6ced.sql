-- Criar função otimizada para dados do perfil
...
RETURNS TABLE(
  ...
  user_cpf varchar,
  user_gerente varchar,
  user_superintendente varchar,
  user_role varchar,
  avatar_url text,
  cover_url text,
  -- Metrics
...
BEGIN
  ...
  ranking_data AS (
    SELECT 
      ...
      ROW_NUMBER() OVER (ORDER BY COUNT(CASE WHEN status IN ('VENDA - SV - VALIDADO DIRETO/NR (SV)', 'VENDA - SV - VALIDADO CEF (SV)') THEN 1 END) DESC) as rank
    FROM sales s
  ...
  user_ranking AS (
    SELECT 
      rank as position,
      (SELECT COUNT(DISTINCT corretor_cpf) FROM sales) as total
    FROM ranking_data 
    WHERE corretor_cpf = user_cpf_var
  )
  SELECT 
...
-- Otimizar a função get_champions_ranking existente para melhor performance
CREATE OR REPLACE FUNCTION public.get_champions_ranking_optimized(
...
BEGIN
  ...
  ELSIF ranking_type = 'gerente' THEN
    ...
    aggregated_data AS (
      SELECT 
        gd.gerente,
        COALESCE(SUM(sd.sales_count), 0) as total_sales,
        COALESCE(SUM(sd.revenue), 0) as total_revenue,
        COALESCE(SUM(vd.visits_count), 0) as total_visits,
        COALESCE(SUM(sd.contracts_count), 0) as total_contracts
  ...
  ELSE -- superintendente
    ...
    aggregated_data AS (
      SELECT 
        sd.superintendente,
        COALESCE(SUM(sales_data.sales_count), 0) as total_sales,
        COALESCE(SUM(sales_data.revenue), 0) as total_revenue,
        COALESCE(SUM(visits_data.visits_count), 0) as total_visits,
        COALESCE(SUM(sales_data.contracts_count), 0) as total_contracts
      FROM super_data sd
