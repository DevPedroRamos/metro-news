...
export const DistratoStats = ({ distratos, userRole }: DistratoStatsProps) => {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3 mb-6">
      ...
      {userRole && valorEspecifico > 0 && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{labelEspecifico}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(valorEspecifico)}</div>
          </CardContent>
        </Card>
      )}
    </div>
